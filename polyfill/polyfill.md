...
> A **polyfill** is code that implements a feature on browsers/engines that do not natively support it. You are re-creating the *exact* behavior of a built-in, manually.
>
...
Array.prototype.myForEach = function (callbackFn, thisArg) {
  ...
  for (let i = 0; i < this.length; i++) {
    if (i in this) { // skip empty (sparse) slots
...
// filter via reduce
const evens = [1,2,3,4].reduce((acc, val) => val % 2 === 0 ? [...acc, val] : acc, []); // [2,4]
```
...
// Test
const onScroll = throttle(() => console.log('Scroll handled at', Date.now()), 200);
...
// Test
promiseAllSettled([Promise.resolve('ok'), Promise.reject('fail')])
  .then(console.log);
```
...
// Test
promiseAny([Promise.reject('e1'), Promise.resolve('ok'), Promise.reject('e2')])
  .then(console.log); // 'ok'
